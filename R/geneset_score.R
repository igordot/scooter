...
geneset_score <- function(module_tbl, counts_raw, min_cpm = 0, limit_pct = 1) {
  ...
  if (!is(counts_raw, "matrix")) {
    stop("expression matrix is not a matrix")
  }
  if (max(counts_raw) < 100) {
    stop("expression values appear to be log-scaled")
  }
  ...
  counts_raw_subs <- normalize_mat_by_gene(
    counts_raw = counts_raw[unlist(module_list), ],
    limit_pct = limit_pct
  )

  # check if enough genes pass filter
  if (min(lengths(module_list)) < 3) {
    stop("too few genes per celltype")
  }
...
filter_mat_by_cpm <- function(counts_raw, min_cpm = 0) {
  # filter matrix by a specified CPM value (higher in at least one
  # sample/column for each gene/row)
...
normalize_mat_by_gene <- function(counts_raw, limit_pct = 1) {
  if (limit_pct > 1) {
    stop("percentile should be expressed as a fraction")
  }
  if (!is(counts_raw, "matrix")) {
    stop("expression matrix is not a matrix")
  }
...
max_scores <- function(scores, method, threshold = 0) {
  # picks the highest-scoring module per cell, or "unknown" if none exceeds
  # threshold method is accepted but not currently used
